package com.github.devlone64.MSLib.util.entity;

import com.github.devlone64.MSLib.MSLib;
...
public class EntityUtil {
    ...
    public static void hideAll(MSPlugin plugin, Entity entity) {
        for (Player loopPlayer : Bukkit.getOnlinePlayers()) {
            loopPlayer.hideEntity(plugin, entity);
        }
    }

    public static void showAll(MSPlugin plugin, Entity entity) {
        for (Player loopPlayer : Bukkit.getOnlinePlayers()) {
            loopPlayer.showEntity(plugin, entity);
        }
    }

    public static void hideAll(MSPlugin plugin, List<Player> players, Entity entity) {
        for (Player loopPlayer : players) {
            loopPlayer.hideEntity(plugin, entity);
        }
    }

    public static void showAll(MSPlugin plugin, List<Player> players, Entity entity) {
        for (Player loopPlayer : players) {
            loopPlayer.showEntity(plugin, entity);
        }
    }
