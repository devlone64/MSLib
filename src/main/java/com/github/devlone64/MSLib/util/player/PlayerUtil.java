package com.github.devlone64.MSLib.util.player;

import com.github.devlone64.MSLib.MSPlugin;
import com.github.devlone64.MSLib.util.player.data.UserData;
...
public class PlayerUtil {
    ...
    public static void hideAll(MSPlugin plugin, Player player) {
        for (Player loopPlayer : Bukkit.getOnlinePlayers()) {
            loopPlayer.hidePlayer(plugin, player);
        }
    }

    public static void showAll(MSPlugin plugin, Player player) {
        for (Player loopPlayer : Bukkit.getOnlinePlayers()) {
            loopPlayer.showPlayer(plugin, player);
        }
    }

    public static void hideAll(MSPlugin plugin, List<Player> players, Player player) {
        for (Player loopPlayer : players) {
            loopPlayer.hidePlayer(plugin, player);
        }
    }

    public static void showAll(MSPlugin plugin, List<Player> players, Player player) {
        for (Player loopPlayer : players) {
            loopPlayer.showPlayer(plugin, player);
        }
    }
